...
export interface Highlight {
  stringIdx: number;            // 0 = highest string (row 0), matches noteClasses order
  fret: number;
...
interface Props {
  ...
  noteClasses?: number[];       // high→low; length = string count. Defaults to bass standard.
...
export default function PracticeFretboard({
  ...
  const STR_COUNT = noteClasses.length;
  ...
  const SVG_H = TOP_PAD + LAST_STR * STR_H + 30;
      ...
      <View style={{ width: SVG_W, height: SVG_H }}>
        <Svg width={SVG_W} height={SVG_H} viewBox={`0 0 ${SVG_W} ${SVG_H}`}>
          ...
          {INLAY_FRETS.map(f => {
            ...
            return f === 12 ? (
              <G key={f}>
                <Circle cx={x} cy={strY(MID - 1)} r={3.5} fill="#252528" />
                <Circle cx={x} cy={strY(MID + 1)} r={3.5} fill="#252528" />
              </G>
            ) : (
              <Circle key={f} cx={x} cy={strY(MID)} r={3.5} fill="#252528" />
            );
          })}
          ...
          <Rect
            ...
            width={NUT_W} height={LAST_STR * STR_H + 20}
            rx={3} fill="#888680"
          />
          ...
          {Array.from({ length: maxFret }, (_, i) => i + 1).map(f => (
            <Line
              ...
              x2={LEFT_PAD + NUT_W + f * FRET_W} y2={strY(LAST_STR) + 8}
