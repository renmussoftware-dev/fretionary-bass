...
function allPositionsFor(noteClass: number, maxFret: number, noteClasses: number[]): Pos[] {
  const out: Pos[] = [];
  for (let s = 0; s < noteClasses.length; s++) {
    for (let f = 0; f <= maxFret; f++) {
      if ((noteClasses[s] + f) % 12 === noteClass) out.push({ stringIdx: s, fret: f });
    }
  }
  return out;
}
...
export default function FindTheNoteDrill({ difficulty, onComplete, onExit }: Props) {
  ...
  const allTargetPositions = useMemo(
    () => allPositionsFor(target, cfg.maxFret, noteClasses),
    [target, cfg.maxFret, noteClasses],
  );
  ...
  const highlights: Highlight[] = useMemo(() => {
    ...
    if (wrongFlash) {
      list.push({
        ...
        kind: 'wrong',
        label: NOTES[(noteClasses[wrongFlash.stringIdx] + wrongFlash.fret) % 12],
      });
    }
    return list;
  }, [foundFrets, wrongFlash, target, noteClasses]);
...
const styles = StyleSheet.create({
  ...
  targetText: { fontSize: 28, fontWeight: '800', color: '#fff', letterSpacing: 1 },
