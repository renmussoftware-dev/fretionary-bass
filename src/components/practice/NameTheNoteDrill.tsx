...
function genQuestion(maxFret: number, naturalsOnly: boolean, noteClasses: number[]): Question {
  const strCount = noteClasses.length;
  for (let attempts = 0; attempts < 50; attempts++) {
    const stringIdx = Math.floor(Math.random() * strCount);
    const fret = Math.floor(Math.random() * (maxFret + 1));
    const noteClass = (noteClasses[stringIdx] + fret) % 12;
    if (naturalsOnly && !NATURAL_NOTES.includes(noteClass)) continue;
    return { stringIdx, fret, noteClass };
  }
  return { stringIdx: 0, fret: 0, noteClass: noteClasses[0] };
}
...
export default function NameTheNoteDrill({ difficulty, onComplete, onExit }: Props) {
  ...
  const noteClasses = useMemo(() => tuningNoteClasses(getTuning(tuningId)), [tuningId]);

  const [qIdx, setQIdx] = useState(0);
  const [question, setQuestion] = useState<Question>(() => genQuestion(cfg.maxFret, cfg.naturalsOnly, noteClasses));
  ...
  // Refs mirror correct + qIdx so async callbacks read the latest values
  // instead of stale closures (otherwise a perfect 10/10 reports as 9/10).
  ...
  function advance() {
    ...
    setQIdx(nextIdx);
    setQuestion(genQuestion(cfg.maxFret, cfg.naturalsOnly, noteClasses));
    setFeedback(null);
